...
class OutcomeAttributionService:
    ...
    3. RecoveryStrategy empirical statistics (attempt_count, success_count, recovery_rate, Wilson LB) are updated.
    ...
    def attribute_verification_result(
        ...
        # Terminal state protection: If case is already RECOVERED or UNRECOVERED
        if case.status in (RecoveryCaseStatus.RECOVERED.value, RecoveryCaseStatus.UNRECOVERED.value) or case.is_terminal:
            logger.info(f"Case '{case.id}' is already in terminal state '{case.status}'. Returning existing status.")
            return {
                "case_id": case.id,
                "status": case.status,
                "outcome": case.status,
        ...
        # 1. Create StrategyOutcome record
        ...
        # 2. Update RecoveryStrategy empirical statistics if segment is assigned
        if case.segment_id:
